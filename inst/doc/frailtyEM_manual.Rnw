...
\end{equation}
with $\alpha >0$ and $\gamma > 0$. This formulation includes several distributions, such as the gamma, positive stable, inverse Gaussian and compound Poisson distributions. This so-called power-variance-function (PVF) family of distributions have been extensively studied in \cite{hougaard2012analysis}. As detailed in Appendix A1, we assume that an identifiability constraint is imposed on the parameters $\alpha$ and $\gamma$ and that the distribution of $Z$ is indexed by a scalar parameter $\theta$.
...
A thorough discussion and comparison of these measures can be found in \cite{hougaard2012analysis}.
...
% The procedure is based on a profile likelihood method and making use of the expectation-maximization (EM) algorithm \cite{dempster1977maximum}.
